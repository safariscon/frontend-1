        ...
        {requests.map((request) => {
          ...
          return <article key={request._id} className="rounded-xl border border-slate-200 p-4">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {request.requestType === 'cancel' && <button type="button" onClick={() => window.alert(`Refund: ${formatRwf(request.refundAmount || 0)}. This is 20% of the paid 30% deposit. Reference: ${request.refundReference || 'Pending'}`)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700">View refund details</button>}
            </div>
