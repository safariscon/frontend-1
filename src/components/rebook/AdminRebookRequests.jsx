  ...
  return <div>
    ...
    <div className="mt-4 grid gap-4">
      {requests.map((request) => {
        ...
        const deposit = Math.min(Number(booking?.amountPaid || 0), Number(booking?.depositAmount || Number(booking?.totalPrice || 0) * 0.3));
        const refundPreview = Math.round(deposit * 0.2);
        ...
        return <article key={request._id} className="rounded-xl border border-slate-200 p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {request.status === 'refund_requested' && <button disabled={busyId === request._id} onClick={() => run(request._id, () => rebookApi.approveRefund(token, request._id))} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">Refund 20% of deposit</button>}
