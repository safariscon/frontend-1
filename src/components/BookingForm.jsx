...
export default function BookingForm({ hotelId, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    return subscribeToRealtime(
      [REALTIME_EVENTS.CATALOG_CHANGED, REALTIME_EVENTS.HOTEL_CHANGED, REALTIME_EVENTS.ROOM_CHANGED],
      loadHotel
    );
  }, [hotelId]);
...
<div className="grid grid-cols-2 gap-4 mb-6">
           <div className="col-span-2">
             <label className="block text-sm font-medium text-gray-700 mb-1">{t('placeToVisit', language)}</label>
             <input
               type="text"
           ...
           <div className="col-span-2">
             <label className="block text-sm font-medium text-gray-700 mb-1">{t('destinationLocation', language)}</label>
             <input
               type="text"
           ...
           <div>
             <label className="block text-sm font-medium text-gray-700 mb-1">{t('checkIn', language)}</label>
             <input
               type="date"
               value={checkIn}
           ...
           <div>
             <label className="block text-sm font-medium text-gray-700 mb-1">{t('checkOut', language)}</label>
             <input
               type="date"
               value={checkOut}
         ...
         <div className="mb-6">
           <label className="block text-sm font-medium text-gray-700 mb-1">{t('guests', language)}</label>
           <select
             value={guests}
           ...
           >
             {[1, 2, 3, 4].map((num) => (
               <option key={num} value={num}>
                 {num} {num === 1 ? t('guest', language) : t('guests_plural', language)}
               </option>
             ))}
           </select>
         </div>
...
{nights > 0 && (
         <div className="bg-gray-50 rounded-xl p-4 mb-6">
           <div className="flex justify-between text-sm mb-2">
             <span className="text-gray-600">{formatRwf(hotel.basePrice)} x {nights} {t('nights', language) || 'nights'}</span>
           ...
           <p className="text-xs text-gray-500 mt-2">
             {t('finalAssignment', language) || 'Final room assignment is completed by admin.'}
           </p>
         </div>
       )}
