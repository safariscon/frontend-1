...
export const toggleAvailableDay = (current, key) => {
  const days = parseDays(current);
  const next = days.includes(key) ? days.filter((item) => item !== key) : [...days, key];
  return next.join(',');
};

export const TIME_REQUIREMENT_OPTIONS = [
  ['auto', 'Automatic (from duration / hours)'],
  ['yes', 'Times required'],
  ['no', 'Dates only — times optional'],
];
...
export const DAY_OPTIONS = DAY_KEYS;
