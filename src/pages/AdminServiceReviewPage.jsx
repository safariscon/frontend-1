...
export default function AdminServiceReviewPage() {
  ...
  const location = {
    latitude: map.latitude ?? service?.serviceLocation?.latitude,
    longitude: map.longitude ?? service?.serviceLocation?.longitude,
    ...
    <DashboardLayout>
      <main className="py-6 sm:py-8">
        <div className="mx-auto max-w-5xl px-4">
          ...
          <div className="mt-3 mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
            ...
            {service && (
              <span className={`w-fit rounded-full px-3 py-1 text-xs font-black uppercase ${approval === 'approved' ? 'bg-emerald-50 text-emerald-700' : approval === 'rejected' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-800'}`}>
                {approval}
              </span>
            )}
          </div>
